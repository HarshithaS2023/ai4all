...
"""
import numpy as np
from app import COLS, build_map, centroids, compute_hwi, compute_pvi, fit, load, nearest
...
assert "PVI" in df.columns and "PVI_percentile" in df.columns, "pandemic vulnerability index missing"
assert "HWI" in df.columns and "HWI_percentile" in df.columns, "heat-wave vulnerability index missing"

pvi = compute_pvi(df)
hwi = compute_hwi(df)
assert pvi["PVI"].notna().all(), "PVI scores contain missing values"
assert hwi["HWI"].notna().all(), "HWI scores contain missing values"
assert set(pvi["top_factor"].dropna().unique()) <= set({"Severe outcome risk (older adults)", "Severe outcome risk (disability)", "Healthcare access barrier", "Household transmission risk", "Density/transmission risk", "Congregate setting risk", "Economic ability to isolate", "Access to testing/care/vaccination"}), "unexpected PVI factor labels"
assert set(hwi["top_factor"].dropna().unique()) <= set({"Older adults", "Children and adolescents", "Disability", "Poverty", "Unemployment", "Limited health care access", "Dense housing / urban form", "Mobile homes", "Crowded housing", "Limited transportation access", "Congregate living"}), "unexpected HWI factor labels"
