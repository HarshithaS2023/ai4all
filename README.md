...
The clustering analysis answers one question: what kinds of counties appear similar when you combine social vulnerability indicators with upward mobility? A second and more operational question is different. If a public health agency wants to understand which counties are likely to be most vulnerable during an infectious-disease or respiratory event, it needs a score that is specific to pandemic conditions rather than a general county-type label.

That is why the app now includes a separate Pandemic Vulnerability Index (PVI). It is not a replacement for the clustering model. It is a complementary view built for a different decision context.

### What the PVI measures

The PVI is a county-level composite score built from SVI indicators that are especially relevant to pandemic vulnerability. It focuses on four broad domains:

- Severe outcome risk: older adults, disability, and lack of insurance
- Household transmission risk: crowded housing, multi-unit housing, and congregate settings
- Ability to isolate or cope: poverty, unemployment, and limited access to transportation or care
- Access to telehealth and information: limited internet access and related barriers

The score is computed by z-scoring each selected indicator against the national county average, then averaging those standardized values. In other words, for county $i$:

$$
PVI_i = \frac{1}{K} \sum_{k=1}^{K} z_{ik}
$$

where $z_{ik}$ is the z-score of the $k$-th pandemic-relevant indicator for county $i$. A county with a PVI near 0 is close to the national average. A positive value means the county is above average on the set of pandemic-relevant vulnerability factors. A negative value means it is below average.

The app also converts the PVI to a percentile rank from 0 to 100 so it is easier to read as a relative standing.

### Which indicators feed the PVI

The PVI uses a practical subset of the SVI variables that map most clearly to pandemic risk. The current implementation includes:

- EP_AGE65: older adults
- EP_DISABL: disability
- EP_UNINSUR: lack of insurance and healthcare access barriers
- EP_CROWD: crowded housing
- EP_MUNIT: multi-unit or dense housing
- EP_GROUPQ: congregate living settings
- EP_POV150: poverty below 150% of the federal poverty line
- EP_UNEMP: unemployment
- EP_NOVEH: limited access to transportation, testing, or care
- EP_NOINT: limited internet access and telehealth barriers

These indicators are not meant to be a literal epidemiological model of infections, hospitalizations, or deaths. They are a structured, county-level vulnerability summary grounded in the CDC/ATSDR SVI framework.

### How to interpret the PVI

The PVI should be read as a relative vulnerability signal, not as a direct forecast.

- A high PVI means the county looks worse than average on several pandemic-relevant vulnerability factors at once.
- A low PVI means it is less burdensome on those same factors.
- The score is especially useful when comparing counties that may look similar in population size or geography but differ in housing, age structure, insurance coverage, or access barriers.

The app also shows the single strongest contributing factor for each county, identified as the indicator with the largest positive z-score. That helps explain what is driving a county’s vulnerability profile, although it should not be treated as the only driver.

### How the PVI differs from the county-type clustering

This is an important distinction.

- The clustering model groups counties into a small number of types based on a broad combination of vulnerability indicators plus upward mobility.
- The PVI does not assign counties to a type. It scores each county on a pandemic-specific vulnerability dimension.
- The clusters are about similarity and pattern recognition.
- The PVI is about severity and operational prioritization.

A county can be high vulnerability on the PVI while still belonging to a cluster that is not obviously “high risk” in the broader social-mobility sense. That is expected. The two views answer different questions.

### Why the PVI was added

The original project was built around county types and whether those types were real. The new PVI layer makes the same data more actionable for a public health workflow:

- identify counties that look unusually vulnerable on pandemic-relevant factors
- compare counties side by side
- inspect which indicators are driving the score
- connect vulnerability to the broader clustering framework without forcing every county into a single label

It is especially useful for planning around outbreaks, resource distribution, or outreach where the question is not “what kind of county is this?” but rather “how exposed does this county look to a pandemic-style disruption?”

### Caveats

The PVI is useful, but it is not a complete public-health risk model.

- It uses county-level indicators, so it cannot capture neighborhood-level disparities.
- It is based on social vulnerability variables, not confirmed cases, hospital capacity, or policy response data.
- It gives equal weight to each selected indicator in the composite score, which is a transparent simplifying choice rather than a claim that every factor is equally important in every outbreak.
- The top-factor display is descriptive, not causal. A county may look high on older-age vulnerability, but that does not mean that factor is the only or even dominant reason it is vulnerable in a real outbreak.

In short, the PVI is a compact, interpretable screening layer on top of the SVI data. It is meant to complement the clustering analysis and make the project more useful for public-health-style questions.

## Heat Wave Vulnerability Index (HWI)

A similar logic can be applied to heat-wave vulnerability. Heat waves are not the same as infectious disease outbreaks, but they also create a situation in which social conditions shape exposure, sensitivity, and ability to cope. A county that is highly exposed to extreme heat may still be much less vulnerable if it has strong infrastructure, more tree cover, better air conditioning access, and healthier living conditions. The goal of a Heat Wave Vulnerability Index (HWI) is to make that distinction explicit.

### What the HWI measures

The HWI would be a county-level composite score built from indicators that are especially relevant to heat exposure and heat-related harm. In practice, a heat-wave vulnerability framework would likely emphasize four broad areas:

- Heat exposure: urban heat island intensity, high summer temperatures, low vegetation, and limited cooling access
- Population sensitivity: older adults, children, disability, and other groups more likely to experience heat stress
- Household coping capacity: housing quality, air conditioning availability, low income, and limited access to transportation or cooling resources
- Environmental and infrastructure context: impervious surface, green space, and neighborhood conditions that affect temperature and recovery

The score would be computed in the same general way as the PVI, by z-scoring each selected indicator against the national county average and then averaging the standardized values. In other words, for county $i$:

$$
HWI_i = \frac{1}{K} \sum_{k=1}^{K} z_{ik}
$$

where $z_{ik}$ is the z-score of the $k$-th heat-relevant indicator for county $i$. A county near 0 would be close to the national average. A positive score would indicate above-average heat-wave vulnerability. A negative score would indicate below-average vulnerability.

### Which indicators could feed the HWI

A heat-wave version of the analysis would likely draw from a mix of environmental and social indicators. Candidates would include:

- EP_AGE65: older adults
- EP_AGE17: children and adolescents
- EP_DISABL: disability
- EP_POV150: poverty and limited financial ability to cool homes
- EP_UNEMP: economic strain and reduced coping capacity
- EP_UNINSUR: limited access to care and health support
- EP_MUNIT: dense housing and urban heat exposure
- EP_MOBILE: mobile homes and reduced thermal resilience
- EP_CROWD: crowding that can intensify heat stress
- EP_NOVEH: limited access to transportation during extreme heat
- EP_NOINT: weaker access to public information or cooling resources
- environmental measures such as tree cover, land surface temperature, or urban heat island intensity where available

These indicators would not represent a full climate-risk model by themselves. They would be a structured screening layer for heat-related vulnerability, based on the same type of county-level logic used for the PVI.

### How to interpret the HWI

The HWI would be read as a relative vulnerability signal rather than a direct forecast of heat deaths or emergency demand.

- A high HWI would suggest that a county looks worse than average on several heat-relevant vulnerability factors at once.
- A low HWI would suggest that the county is less burdened on those same factors.
- The index would be especially useful when comparing counties that are similar in size or geography but differ in housing quality, age structure, income, or environmental exposure.

As with the PVI, the app could also display the single strongest contributing factor for each county, identified as the indicator with the largest positive z-score.

### How the HWI differs from the county-type clustering

The distinction would mirror the one used for the PVI.

- The clustering model groups counties into types based on broad social vulnerability patterns and mobility.
- The HWI would not assign counties to a type. It would score each county on a heat-specific vulnerability dimension.
- The clusters are about similarity and pattern recognition.
- The HWI would be about heat exposure and coping capacity.

A county could be highly vulnerable on heat while still belonging to a cluster that does not look obviously extreme in the broader social-mobility sense. That would be expected.

### Why an HWI would be useful

A heat-wave version of this framework would make the project more useful for climate adaptation planning and public-health preparedness:

- identify counties that look unusually vulnerable to extreme heat
- compare counties side by side
- inspect which indicators are driving the score
- connect heat vulnerability to the broader county-level vulnerability framework without forcing every county into a single label

It would be especially relevant for emergency planning, cooling-center outreach, housing resilience programs, and public-health messaging during heat events.

### Caveats

The HWI would be useful, but it would not be a complete heat-risk model.

- It would rely on county-level indicators, so it could miss neighborhood-level disparities.
- It would be based on vulnerability proxies rather than direct heat-health outcomes.
- It would likely give equal weight to each selected indicator unless a more detailed weighting scheme were introduced.
- The top-factor display would be descriptive, not causal.

In short, an HWI would function as a compact, interpretable screening layer for heat-wave vulnerability, parallel to the PVI and designed to complement the clustering analysis.
