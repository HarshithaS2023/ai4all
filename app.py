...
COLS = list(FEATS)
PVI_VARS = {
    "EP_AGE65": "Severe outcome risk (older adults)",
    "EP_DISABL": "Severe outcome risk (disability)",
    "EP_UNINSUR": "Healthcare access barrier",
    "EP_CROWD": "Household transmission risk",
    "EP_MUNIT": "Density/transmission risk",
    "EP_GROUPQ": "Congregate setting risk",
    "EP_POV150": "Economic ability to isolate",
    "EP_UNEMP": "Economic ability to isolate",
    "EP_NOVEH": "Access to testing/care/vaccination",
}
HEAT_VARS = {
    ...
    "EP_UNINSUR": "Limited health care access",
    "EP_MUNIT": "Dense housing / urban form",
    "EP_MOBILE": "Mobile homes",
    "EP_CROWD": "Crowded housing",
    "EP_NOVEH": "Limited transportation access",
    "EP_GROUPQ": "Congregate living",
}
...
def compute_index(df: pd.DataFrame, variable_map: dict, score_col: str, percentile_col: str):
    """Create a composite vulnerability score from available indicators."""
    out = df.copy()
    available = [var for var in variable_map if var in out.columns]
    if not available:
        out[score_col] = np.nan
        out[percentile_col] = np.nan
        out["top_factor"] = np.nan
        out["top_factor_zscore"] = np.nan
        return out

    z = (out[available] - out[available].mean()) / out[available].std()
    out[score_col] = z.mean(axis=1)
    out[percentile_col] = out[score_col].rank(pct=True) * 100
    out["top_factor"] = z[available].idxmax(axis=1).map(variable_map)
    out["top_factor_zscore"] = z[available].max(axis=1).round(2)
    return out


def compute_pvi(df: pd.DataFrame) -> pd.DataFrame:
    return compute_index(df, PVI_VARS, "PVI", "PVI_percentile")


def compute_hwi(df: pd.DataFrame) -> pd.DataFrame:
    return compute_index(df, HEAT_VARS, "HWI", "HWI_percentile")
...
def load_geojson():
    ...
    with urlopen(GEOJSON, context=context) as resp:
...
def render_vulnerability_view(df, score_col, title, subtitle, variable_map):
    st.title(title)
    st.caption(subtitle)

    state_filter = st.multiselect(
        "Filter to state(s) (optional)",
        options=sorted(df["ST_ABBR"].dropna().unique()),
    )
    plot_df = df if not state_filter else df[df["ST_ABBR"].isin(state_filter)]
    names = sorted(plot_df.name)
    if not names:
        st.info("No counties match that state filter.")
        st.stop()

    selected_key = f"{score_col.lower()}_sel"
    if selected_key not in st.session_state or st.session_state[selected_key] not in names:
        st.session_state[selected_key] = names[0]
    selected = st.selectbox("Selected county", names, key=selected_key)
    ...
    fig = px.choropleth(
        ...
        range_color=(plot_df[score_col].quantile(0.02), plot_df[score_col].quantile(0.98)),
        scope="usa",
        hover_name="name",
        hover_data={"FIPS": False, "ST_ABBR": True, score_col: ":.2f", "E_TOTPOP": ":,", "top_factor": True},
        labels={score_col: title, "top_factor": "Top factor", "E_TOTPOP": "Population"},
    )
    fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), height=560)
    st.plotly_chart(fig, use_container_width=True)

    row = plot_df.loc[plot_df.name == selected].iloc[0]
    z = (df[list(variable_map)] - df[list(variable_map)].mean()) / df[list(variable_map)].std()
    factor_df = pd.DataFrame({
        "Factor": [variable_map[v] for v in z.columns],
        "z-score": z.loc[row.name].values,
    }).sort_values("z-score")

    bar = px.bar(
        factor_df, x="z-score", y="Factor", orientation="h",
        color="z-score", color_continuous_scale="RdYlBu_r", range_color=(-3, 3),
        title=f"What is driving {selected}?",
    )
    bar.update_layout(height=420, coloraxis_showscale=False)
    st.plotly_chart(bar, use_container_width=True)

    col1, col2, col3 = st.columns(3)
    col1.metric(title, f"{row[score_col]:.2f}", help="Average z-score across the selected vulnerability indicators. Positive values are above the national county average.")
    col2.metric("Percentile rank", f"{row[f'{score_col}_percentile']:.0f}th")
    col3.metric("Top factor", row["top_factor"])
    st.caption("z-scores are standardized against the national county average; positive values indicate higher-than-average vulnerability on that factor.")


def main():
    df = load()

    view = st.sidebar.radio("Explore", ["County types", "Pandemic vulnerability", "Heat wave vulnerability"], index=0)

    if view == "Pandemic vulnerability":
        render_vulnerability_view(
            df,
            "PVI",
            "County-level pandemic vulnerability",
            "Use the county picker to inspect the composite Pandemic Vulnerability Index and the factors driving it.",
            PVI_VARS,
        )
        st.stop()

    if view == "Heat wave vulnerability":
        render_vulnerability_view(
            df,
            "HWI",
            "County-level heat wave vulnerability",
            "Use the county picker to inspect the composite Heat Wave Vulnerability Index and the factors driving it.",
            HEAT_VARS,
        )
        st.stop()
